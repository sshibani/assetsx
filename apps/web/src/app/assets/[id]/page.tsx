...
/** Read-only EXIF / camera details panel, including a GPS map when present. */
function ExifPanel({ metadata }: { metadata: ImageMetadataDTO }) {
  const m = metadata;
  const captured =
    m.capturedAt &&
  ...
  return (
    <div className="panel" style={{ marginTop: 20 }}>
      <h3 className="section-title">Camera / EXIF details</h3>
      <ExifRow label="Captured" value={captured || null} />
      <ExifRow label="Camera" value={[m.cameraMake, m.cameraModel].filter(Boolean).join(" ") || null} />
